...
import { FabricFormDialog, ColorFormDialog, RollFormDialog, type FabricFormState, type ColorFormState, type RollFormState } from "@/components/inventory/InventoryDialogs";
import { ConfirmDelete, type DeleteTarget, type BulkDeleteItem } from "@/components/inventory/ConfirmDelete";
    ...
    >
      ...
      <div className="rounded-xl border border-border bg-card shadow-soft overflow-hidden">
        {filteredFabrics.length === 0 && (
          <div className="px-6 py-12 text-center text-sm text-muted-foreground">لا نتائج مطابقة للبحث.</div>
        )}
                      ...
                      return (
                        <li key={c.id}>
                          <ColorRow
                            color={c}
                            open={colorOpen}
                            onToggle={() => setExpandedColor((cur) => (cur === c.id ? null : c.id))}
                            onEdit={() => setColForm({ open: true, fabricId: f.id, editing: c })}
                            onDelete={() => setToDelete({ kind: "color", id: c.id, name: c.name })}
                            onAddRoll={() => setRolForm({ open: true, colorId: c.id })}
                            selectable={selectMode}
                            selected={!!selected[selKey({ kind: "color", id: c.id, name: `${c.name} — ${c.code}` })]}
                            onSelect={() => toggleSelect({ kind: "color", id: c.id, name: `${c.name} — ${c.code}` })}
                          />
                          {colorOpen && (
                            <ul>
                              {rollsOfColor(c.id).map((r) => (
                                <li key={r.id}>
                                  <RollRow
                                    ...
                                    onEdit={() => setRolForm({ open: true, colorId: c.id, editing: r })}
                                    onDelete={() => setToDelete({ kind: "roll", id: r.id, name: `#${r.rollNo}` })}
                                    selectable={selectMode}
                                    selected={!!selected[selKey({ kind: "roll", id: r.id, name: `#${r.rollNo}` })]}
                                    onSelect={() => toggleSelect({ kind: "roll", id: r.id, name: `#${r.rollNo}` })}
                                  />
                                </li>
                              ))}
                            </ul>
      ...
      <ConfirmDelete target={toDelete} onCancel={() => setToDelete(null)} onConfirm={handleDelete} />
