...
function NewExpense() {
  ...
  const save = () => {
    setErr(null);
    let valid = true;
    if (!category.trim()) { setCatError("اسم المصروف مطلوب."); valid = false; }
    const num = Number(amount);
    if (amount === "" || isNaN(num) || num <= 0) { setAmtError("أدخل مبلغاً صحيحاً أكبر من صفر."); valid = false; }
    if (!description.trim()) { setDescError("الوصف مطلوب."); valid = false; }
    if (!valid) return;
    create.mutate(
      {
        category,
      ...
      },
      {
        onSuccess: (res) => {
          ...
          else setErr((res.error as any)?.message ?? (typeof res.error === "string" ? res.error : "فشل إنشاء المصروف"));
        },
      },
    );
  };
      ...
      >
        <div className="grid gap-3 md:grid-cols-3">
          <FormField label="اسم المصروف *" error={catError ?? undefined}>
            <NameCombobox
              value={category}
              onChange={(v) => { setCategory(v); setCatError(null); }}
              options={names}
          ...
          <FormField label="المبلغ *" error={amtError ?? undefined}>
            <Input
              type="number"
              value={amount}
              onChange={(e) => { setAmount(e.target.value === "" ? "" : Number(e.target.value)); setAmtError(null); }}
              className="h-11"
            />
          </FormField>
